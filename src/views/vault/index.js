...
import { getCirculatingSupply, getTotalSupply, getBalance } from '../../yzy/token';
import {
...
function Vault() {
    ...
    useEffect(async () => {
        if (userSwapReward.pending && userSwapReward.pending.isGreaterThan(new BigNumber(0))) {
            const wethEstimateAmount = await getAmountOut(yzyETHPairContract, bnToDec(userSwapReward.pending), true);
    ...
    const onClaimAvailableYzyReward = async (event) => {
        ...
        const encodedABI = vaultContract.contract.methods.claimYzyAvailableReward().encodeABI();
    ...
    const onClaimYzyReward = async (event) => {
        ...
        const encodedABI = vaultContract.contract.methods.claimYzyReward().encodeABI();
                        ...
                        <Row>
                            <Col xs={12} sm={4}>
                                <Form
                                    title='TOTAL SUPPLY'
                                >
                                    <span className="numberSpan">{bnDivdedByDecimals(totalSupply).toFormat(4)} YZY</span>
                                </Form>
                            </Col>
                            <Col xs={12} sm={4}>
                                <Form
                                ...
                                >
                                    <span className="numberSpan">{bnDivdedByDecimals(circulatingSupply).toFormat(4)} YZY</span>
                                </Form>
                            </Col>
                            <Col xs={12} sm={4}>
                                <Form
                                    title='YOUR YZY BALANCE'
                                >
                                    <span className="numberSpan">{bnDivdedByDecimals(userBalance).toFormat(4)} YZY</span>
                                </Form>
                            </Col>
                        </Row>
                        <Row>
                            ...
                            <Col xs={12} sm={4}>
                                <Form
                                    title='YZY PRICE'
                                >
                                    <span className="numberSpan">${new BigNumber(yzyPrice).toFormat(2)}</span>
                                </Form>
                            </Col>
                            <Col xs={12} sm={4}>
                                <Form
                                        ...
                                        <Col xl={12}>
                                            <Row>
                                                <Col xl={12}>
                                                    <Label
                                                        label='Pending YZY'
                                                        balance={userYzyReward.pending ? bnDivdedByDecimals(userYzyReward.pending).toFormat(4) + ' ($' + bnDivdedByDecimals(pendingYZYValue).toFormat(2) + ')' : 0 + '($0)'}
                                                    />
                                                </Col>
                                                <Col xl={12}>
                                                    <Label
                                                        label='Available YZY'
                                                        balance={userYzyReward.available ? bnDivdedByDecimals(userYzyReward.available).toFormat(4) + ' ($' + bnDivdedByDecimals(availableYZYValue).toFormat(2) + ')' : 0 + '($0)'}
                                                    />
                                                </Col>
                                                <Col xl={12}>
                                                    <Button onClickHandler={onClaimAvailableYzyReward} color='yellow'>Claim Available</Button>
                                                </Col>
                                                <Col xl={12}>
                                                    <Button onClickHandler={onShowConfirmModalForYZY} color='yellow'>Claim Early</Button>
                                                </Col>
                                            </Row>
                                        </Col>
                                    </Row>
